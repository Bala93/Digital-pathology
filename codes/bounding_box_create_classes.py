...
def write_xml(root,xmin,ymin,xmax,ymax,width,height,nuclei_class):
...
if __name__ == "__main__":
    ...
    Example : 
    python bounding_box_create_classes.py --mask_path='/media/htic/NewVolume1/murali/mitosis/mitotic_count/mask_size_512_stride_128_updated' --mask_ext='jpg' --xml_path='/media/htic/NewVolume1/murali/mitosis/512_128_xml'
    ...
    for each_mask_path in tqdm(mask_images_path):
        ...
        img_red = img_mask[:,:,2]
        img_green = img_mask[:,:,1]
        
        # RED(MITOTIC)
        cnts_flag_red = 0
        area_flag_red = 0
        nuclei_class = "Mitotic"
        _,im_bw_red = cv2.threshold(img_red, 200, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        kernel = np.ones([5,5],np.uint8)
        # opening = cv2.morphologyEx(im_bw,cv2.MORPH_OPEN,kernel)
        dilation_red = cv2.dilate(im_bw_red,kernel,iterations=3)
        cnts = cv2.findContours(dilation_red, cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
        ...
        for c in cnts:
            ...
            if area_ > min_area:
                ...
                for obj in objs:
                    # print("1:",obj.find('name').text)
                    if obj.find('name').text == nuclei_class:
                        node = et.Element('bndbox')
                        obj.insert(1,node)
                        # write_xml(root,xmin,ymin,xmax,ymax,width,height,nuclei_class)
        ...
        cnts_flag_green = 0
        area_flag_green = 0
        nuclei_class = "NonMitotic"
        _,im_bw_green = cv2.threshold(img_green, 200, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        kernel = np.ones([5,5],np.uint8)
        # opening = cv2.morphologyEx(im_bw,cv2.MORPH_OPEN,kernel)
        dilation = cv2.dilate(im_bw_green,kernel,iterations=3)
        ...
        for c in cnts:
            ...
            break
        #Write to xml only for files which return a bounding box.        
        if((area_flag_red and cnts_flag_red) or (area_flag_green and cnts_flag_green)):
            tree.write(os.path.join(xml_path,xml_name))
