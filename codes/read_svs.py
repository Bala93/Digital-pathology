...
dir_img = '/media/htic/Seagate Backup Plus Drive/ICIAR/WSI/'
...
for f in tqdm(os.listdir(dir_img)):
    ...
    for r in tqdm(range(0,orig_w,patch_size[0])):
        for c in tqdm(range(0, orig_h,patch_size[1])):
    ...
    img_np = img_np[::downsample_factor,::downsample_factor,:]
    
    # if save:
    #     name_no_ext = os.path.splitext(f)[0]
    #     np.save(dir_img + name_no_ext, img_np)
    name_no_ext = os.path.splitext(f)[0]
    save_path = os.path.join(dir_img,name_no_ext + '.png')
    imsave(save_path,img_np)
    scan.close
    # break
